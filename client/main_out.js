(function (wHandle, wjQuery) {
            ...
            switch (event.keyCode) {
                ...
                case 13:
                    if (isTyping) {
						...
						//bad words list
                        var bwlist = new Array('ㅗ', 'ko', 'koagar', 'koagario', '씨발', 'agar', 'www', '서버', 'ㅅㅂ', 'ㅆㅂ', '병신', 'ㅄ', 'ㅂㅅ', '존나', 'ㅈㄴ', '애미', '에미', 'ㄴㄱㅁ', '느금마', '창년', '새끼', '보지', '자지', '간나', '개새끼', '게이', '고자', '니기미', 'Fuck', 'Motherfucker', '닥쳐', '등신', '또라이', '똘추', '닭대가리', '루저', '매국노', '미친놈', '미친년', '미친새끼', '변태', '빌어먹을', '빠가', '싸이코', '사이코', '시바사키', '시팔', '씨팔', '썅', '싸가지', '씨발년', '씨발놈', '엠창', '염병', '옘병', '애미', '애비', '엿', '인간쓰레기', '인간말종', '제기랄', 'Damn it', '좆까', '지랄', '찌질이', '짱깨', '변태', '호모', '충', '호구', '홍어', 'ass', 'Ball', 'Bitch', 'Bullshit', 'Damn', 'Dick', 'Fuck', 'Gay', 'Goddamn', 'Holy', 'Lame', 'Loser', 'Nigger', 'Nuts', 'Psycho', 'Pussy', 'Retard', 'Shit', 'Suck', 'Stupid', 'Willie', 'Vagina');
						var banned = false;
						
						for (var i in bwlist){
							if(document.getElementById("chat_textbox").value.indexOf(bwlist[i]) > -1) {
								banned = true;
								break;
							}
						}
						
						//if (document.getElementById("chat_textbox").value.search("씨발") > 0) {
                        if (banned) {
							alert("바른말을 사용합시다.");
                        ...
                        if (chattxt.length > 1) sendChat(chattxt);
                            document.getElementById("chat_textbox").value = "";
						    chat_blocked = true;
						} else {
							document.getElementById("chat_textbox").value =  "";
							document.getElementById("chat_textbox").blur();	
							alert("도베로 인하여 전송되지 않았습니다. 도배를 계속할시에는, 벤을 당하실수 있습니다.");
						}
						}
                    }
                    else {
    ...
    function getServerList() {
        ...
        wjQuery.get("info", function (a) {
            ...
            for (var numplayer in numPlayers) wjQuery('#region option[value="' + numplayer + '"]').text(playerStat[numplayer])
        }, "json")
    }
    ...
    function drawGameScene() {
        ...
        chatCanvas && ctx.drawImage(chatCanvas, 0, canvasHeight - chatCanvas.height - 50); // draw Leader Board
